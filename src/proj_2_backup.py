...
class VelmitronoManipulinator:
    ...
    def change_gripper_state(self, side="right", is_holding=False, state=None, angle=None):
        ...
        if self.gripper_state(side) or state=="close":
            if angle is None:
                angle = 100
        else:
            angle = 0
        q = [math.radians(angle), math.radians(angle), math.radians(angle), math.radians(0)]
        if side == "right":
    ...
    # def cart_move(self, object, hand, distancex=0.0,distancey=0.0,distancez=0.0, up=False): #prawa graba 12 normalnie 5 bo octo
    #     rospy.sleep(5)
    ...
    def get_PYKDL_frame(self, object, x, y, z):
        return PyKDL.Frame(PyKDL.Rotation.Quaternion( 0.0 , 0.0 , 0.0 , 1.0 ), PyKDL.Vector(object.p.x()+x, object.p.y()+y , object.p.z()+z))

    def moveCardImp(self, hand, transform, NM, NR, time=0.5):
        self.velma.moveCartImp(hand, [transform], [3.0], None, None, None, None, PyKDL.Wrench(PyKDL.Vector(NM, NM, NM), PyKDL.Vector(NR, NR, NR)), start_time=time)
...
def main():
    ...
    # loading objects
    # box = s.velma.getTf("B", "right_handle")
    # box_starting = s.velma.getTf("B", "right_handle")
    # print("klamka: ",box)
    # box2 = s.velma.getTf("B", "right_door")
    # print("drzwi: ",box2)

    handle_R = s.velma.getTf("B", "right_handle")
    handle_L = s.velma.getTf("B", "left_handle")
    door_R = s.velma.getTf("B", "right_door")
    door_L = s.velma.getTf("B", "left_door")

    handle_distance_R = math.sqrt(handle_R.p.x()**2+handle_R.p.y()**2)
    handle_distance_L = math.sqrt(handle_L.p.x()**2+handle_L.p.y()**2)

    # position of handle
    T_B_Th = None
    if handle_distance_R < handle_distance_L:
        twe, handtype = s.transformations_object("left_handle",x=0.4,y=0.0,z=-0.08, hand="left") 
        handle = handle_L
        param = -1 
    else:
        twe, handtype = s.transformations_object("right_handle",x=0.4,y=0.0,z=-0.08, hand="right") 
        handle = handle_R
        param = 1

    print("TWE: ", twe, "Hand: ", handtype)
    ...
    #rotating hand
    print("Rotating writs by 90 deg around local y axis (hand side matrix multiplication)")
    if handtype == "right":
        T_B_Th = s.velma.getTf("B", "Tr")
        print(T_B_Th)
        T_B_Trd = T_B_Th * PyKDL.Frame(PyKDL.Rotation.RotY((-90.0)/180.0*math.pi))
    else:
        T_B_Th = s.velma.getTf("B", "Tl")
        print(T_B_Th)
        T_B_Trd = T_B_Th * PyKDL.Frame(PyKDL.Rotation.RotY((-270.0)/180.0*math.pi))
    print(T_B_Trd)
    s.moveCardImp(handtype, T_B_Trd, 5, 5)

    #closing gripp a little big   
    print("closing gripp a little bit")
    s.change_gripper_state(handtype, angle=70)
    #getting closer
    print("getting closer")
    rospy.sleep(1)
    T_B_Trd = s.get_PYKDL_frame(handle, -0.28, param*-0.02, 0.14)
    s.moveCardImp(handtype, T_B_Trd, 1, 0.01)
    rospy.sleep(5)

    #closing gripper and getting handle
    print("closing gripper and getting handle")
    s.change_gripper_state(handtype, state="close", is_holding=True)
    
    
    #backin a lil bit with handle - 75 deg
    #calculating offset
    deg = 30
    side = 0.585/2
    c = math.sqrt(2* side*side * (1-math.cos(deg)))
    alph = (180-deg)/2
    dx= c * math.sin(alph)
    dy = c * math.cos(alph)
    z = 0.14
    ...
    print("backing up")
    handle = s.velma.getTf("B", "right_handle")
    T_B_Trd = s.get_PYKDL_frame(handle, -0.28+dx, param*(0.15-dy), z)
    s.moveCardImp(handtype, T_B_Trd, 18, 0.1)
    rospy.sleep(2)
    print("backing up finished")

    #letting go of handle and backing away
    print("releasing handle")
    s.change_gripper_state(handtype, angle=60)
    rospy.sleep(2)

    print("releasing handle finished")
    print("backing up more")
    handle = s.velma.getTf("B", "right_handle")
    T_B_Trd = s.get_PYKDL_frame(handle, -0.28-0.1, param*(-0.01-0.1), z)
    s.moveCardImp(handtype, T_B_Trd, 18, 0.1)
    rospy.sleep(2)

    print("backing up more finished")
    #going around
    print("closing arm")
    s.change_gripper_state(handtype, state="close", is_holding=True)
    rospy.sleep(2)

    print("closing arm finished")
    print("going around")
    T_B_Trd = s.get_PYKDL_frame(handle, -0.28-0.05, param*0.28, z)
    s.moveCardImp(handtype, T_B_Trd, 5, 5)
    rospy.sleep(2)

    print("going around finished")
    #hit door
    print("forward")
    T_B_Trd = s.get_PYKDL_frame(handle, -0.28+0.1, param*0.28, z)
    s.moveCardImp(handtype, T_B_Trd, 5, 5)
    rospy.sleep(2)

    print("forward finished")
    print("hit door")
    T_B_Trd = s.get_PYKDL_frame(handle, -0.05-side/2 , param*(-side/2 -0.3), z)
    s.moveCardImp(handtype, T_B_Trd, 5, 5)
    rospy.sleep(2)

    print("hit door finished")
    #homing - back from door and go joint mode
    print("backing after hit")
    T_B_Trd = s.get_PYKDL_frame(handle, -0.28-0.1, param*(-side/2 +0.1), z)
    s.moveCardImp(handtype, T_B_Trd, 5, 5)
    rospy.sleep(2)

    print("backing after hit finished")
    s.joint_mode()
    s.homing()
